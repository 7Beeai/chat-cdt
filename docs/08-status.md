...
> **Atualizar ao fim de cada sessão de trabalho.** Última atualização: 2026-06-12 (histórico Motor V2).
>
> **Sessão 2026-06-12 (histórico do motor RELIGADO)** — os 27 disparadores do Motor V2 (14 cobrança + 13 relacionamento) foram instrumentados no n8n e todas as franquias voltaram a gravar cada envio em `message_log`, agora com **`mensagem_texto`** (corpo do template já renderizado, {{n}} substituídos). Migration **0020** substitui `chat_cadence_history` por **`chat_motor_history`**: corpo direto de `mensagem_texto` (inventory vira fallback raro), corte em 2026-06-12 (nunca retroativo, decisão de docs/13), `disparos_log` removido (0 linhas em prod, nunca foi populado), e inbound de `message_inbound` como rede de segurança com **dedup global por wamid** (verificado: todo "gap" por-conversa era mensagem em outra conversa do mesmo contato — dedup por conversa duplicaria). Índice novo `idx_mi_unit_matchkey`. A thread volta a mostrar os disparos da régua (flag `SHOW_CADENCE_HISTORY` removida). Unidades aparecem no log a partir do próximo disparo de cada uma. **Atenção: patrocinio001 tem inbound em `message_inbound` mas ZERO no chat — webhook da Meta não está chegando pro CHAT-CDT nessa WABA (subscrição/whitelist pendente).**
>
